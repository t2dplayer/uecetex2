    ...
    \\begin{columns}[T]
        ...
        \\begin{column}{0.49\\textwidth}
            ...
            \\begin{tikzpicture}[>=stealth, scale=0.75, transform shape, font=\\sffamily]
                ...
                \\node[snode, fill=orange!10, draw=orange!80!black] at (1.0, 1.25) {$R_1$};
                \\node[snode, fill=orange!10, draw=orange!80!black] at (2.5, 1.25) {$R_2$};
                \\node[snode, fill=orange!10, draw=orange!80!black] at (4.0, 1.25) {$R_3$};
                \\node[snode, fill=red!10, draw=red!70!black, minimum width=2.2cm]
                      (mg) at (2.5, 0.65) {MERGE};
                \\draw[->, thick, orange!80!black] (1.0, 0.97) -- (mg.north west);
                \\draw[->, thick, orange!80!black] (2.5, 0.97) -- (mg.north);
                \\draw[->, thick, orange!80!black] (4.0, 0.97) -- (mg.north east);
                \\node[text=red!80!black, font=\\scriptsize\\bfseries] at (2.5, 0.20)
                      {OVERHEAD DE SINCRONIA};
                \\draw[dashed, gray!40] (0, -0.05) -- (5.0, -0.05);

                % Tolerância a Falhas
                \\node[text=orange!75!black, font=\\scriptsize\\bfseries] at (2.5, -0.30) {Tolerância a Falhas};
                \\node[snode, fill=orange!10, draw=orange!80!black] at (1.0, -0.82) {$T_1$\\,\\checkmark};
                \\node[snode, fill=red!10,    draw=red!80!black]    at (2.5, -0.82) {$T_2$\\,\\texttimes};
                \\node[snode, fill=orange!10, draw=orange!80!black] at (4.0, -0.82) {$T_3$\\,\\checkmark};
                \\node[text=red!80!black, font=\\scriptsize\\bfseries] at (2.5, -1.45)
                      {REEXEC $T_2$};
            \\end{tikzpicture}
        \\end{column}
    \\end{columns}
