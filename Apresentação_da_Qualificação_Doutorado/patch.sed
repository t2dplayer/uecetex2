...
\
                ...
                \node[text=red!80!black, font=\scriptsize\bfseries] at (2.5, 2.02) {SINCRONIA};\
                \draw[dashed, gray!40] (0, 1.90) -- (5.0, 1.90);\
\
                % Overhead Parcial\
                \node[text=orange!75!black, font=\scriptsize\bfseries] at (2.5, 1.65) {Overhead Parcial};\
                \node[snode, fill=orange!10, draw=orange!80!black] at (1.0, 1.15) {$R_1$};\
                \node[snode, fill=orange!10, draw=orange!80!black] at (2.5, 1.15) {$R_2$};\
                \node[snode, fill=orange!10, draw=orange!80!black] at (4.0, 1.15) {$R_3$};\
                \node[snode, fill=red!10, draw=red!70!black, minimum width=2.2cm]\
                      (mg) at (2.5, 0.55) {MERGE};\
                \draw[->, thick, orange!80!black] (1.0, 0.87) -- (mg.north west);\
                \draw[->, thick, orange!80!black] (2.5, 0.87) -- (mg.north);\
                \draw[->, thick, orange!80!black] (4.0, 0.87) -- (mg.north east);\
                \node[text=red!80!black, font=\scriptsize\bfseries] at (2.5, 0.15)\
                      {OVERHEAD DE SINCRONIA};\
                \draw[dashed, gray!40] (0, 0.35) -- (5.0, 0.35);\
\
                % Tolerância a Falhas\
                \node[text=orange!75!black, font=\scriptsize\bfseries] at (2.5, 0.10) {Tolerância a Falhas};\
                \node[snode, fill=orange!10, draw=orange!80!black] at (1.0, -0.42) {$T_1$\,\checkmark};\
                \node[snode, fill=red!10,    draw=red!80!black]    at (2.5, -0.42) {$T_2$\,\texttimes};\
                \node[snode, fill=orange!10, draw=orange!80!black] at (4.0, -0.42) {$T_3$\,\checkmark};\
                \node[text=red!80!black, font=\scriptsize\bfseries] at (2.5, -1.05)\
                      {REEXEC $T_2$};\
            \end{tikzpicture}\
        \end{column}\
    \end{columns}\
