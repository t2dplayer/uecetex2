...
1. Entidades (unidade processadora, tarefa)
    ...
    1.4 Protocolo de Offloading, na camada de aplicação
        1.4.1 Pedido de Offload
        ...
        1.4.8 Considerações sobre Simulador de Eventos Discretos
            1.4.8.1 Enviar pedido de offload
...
3. Task Offloading
    3.1 Modos de Execução
        ...
        3.1.2 Offloading V2I (para RSU via 5G NR)
        3.1.3 Offloading V2V (para veículo vizinho via Sidelink)
    3.2
4. Coleta de métricas de desempenho do task offloading
    4.1 Métricas Globais
        ...
        4.1.4 Taxa de Sucesso no Offloading
        4.1.5 Total de Tarefas Processadas Localmente
        4.1.6 Total de Tarefas Offloadadas via V2I
        4.1.7 Total de Tarefas Offloadadas via V2V
    4.2 Métricas por Política de Offloading
        4.2.1 Atraso Médio por Política
        4.2.2 Consumo Energético Médio por Política
        4.2.3 Taxa de Sucesso no Offloading por Política
...
6. Políticas de Decisão de Offloading
    6.1 Políticas Heurísticas
        6.1.1 Processamento Local
        6.1.2 Offloading Aleatório
        6.1.3 Offloading Ganancioso
...
6. Criação de testes unitários para validação dos módulos desenvolvidos
    ...
    6.3 Testes para o módulo de offloading
    6.4 Testes para o protocolo de descoberta de vizinhança
    6.5 Testes para coleta de métricas de desempenho do task offloading
...
9. Criar logs para facilitar o rastreamento da execuçao offloading (fim a fim)
...
%     \item \textbf{Protocolo de Offloading:} Um protocolo de comunicação baseado em UDP foi definido para gerenciar a negociação. O fluxo consiste em \texttt{TASK\_REQUEST} (pedido de offload), \texttt{TASK\_RESPONSE} (aceite ou rejeição, ex: \texttt{REJECTED\_QUEUE\_FULL}), \texttt{TASK\_DATA} (envio dos dados da tarefa), \texttt{TASK\_RESULT} (retorno do resultado) e \texttt{TASK\_ACK} (confirmação).
% \end{itemize}
