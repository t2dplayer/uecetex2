...
A metodologia deste trabalho é fundamentada na simulação, utilizando o simulador de redes NS-3, estendido com os módulos 5G-LENA e C-\gls{V2X} para modelar com fidelidade o ambiente veicular 5G.
...
1. Entidades (unidade processadora, tarefa)
    ...
    1.5 Protocolo de Descoberta de Vizinhança, na camada de aplicação
        1.5.1 Emissão de Beacons Periódicos (via \gls{5G} \gls{NR} e \gls{V2X} \textit{Sidelink})
...
2. Canal de comunicação
    ...
    2.2 Modelo de Canal \gls{V2X} \textit{Sidelink} (\gls{PC5})
...
3. Descarregamento de tarefas
    3.1 Modos de Execução
        ...
        3.1.3 Descarregamento \gls{V2V} (para veículo vizinho via \textit{Sidelink})
    3.2
4. Coleta de métricas de desempenho do descarregamento de tarefas
    ...
    4.3 Análise de Sensibilidade
        ...
        4.3.2 \textit{Trade-off} entre Latência, Consumo Energético e Justiça
...
% O sistema permite três modos de execução: processamento local, offloading \gls{V2I} (para \gls{RSU} via 5G) e offloading \gls{V2V} (para veículo vizinho via Sidelink).
