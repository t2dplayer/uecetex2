...
1. Entidades (unidade processadora, tarefa)
    1.1 Unidade Processadora (\gls{OBU} e \gls{VEC})
        ...
        1.1.3 Frequencia do Clock (GHZ)
            1.1.3.1 Modulo para criar perfil heterogeneo de \gls{CPU} (maquinas lentas, medias, rapidas)
    ...
    1.2 Tarefa
        ...
        1.2.4 Densidade de Carga Computacional (tamanho maximo e minimo dependente do perfil de aplicação - ciclos de \gls{CPU} por byte)
        1.2.5 Densidade de Carga Energética (tamanho maximo e minimo dependente do perfil de aplicação - joules por ciclo de \gls{CPU})
    ...
    1.4 Protocolo de descarregamento, na camada de aplicação
        ...
        1.4.2 Resposta ao Pedido (Aceite/Rejeição)
            ...
            1.4.2.3 Rejeição por não cumprimento do \textit{deadline}
    ...
    1.5 Protocolo de Descoberta de Vizinhança, na camada de aplicação
        1.5.1 Emissão de Beacons Periódicos (via \gls{5G} \gls{NR} e \gls{V2X} Sidelink)
            1.5.1.1 Conteúdo do Beacon (Device Specs, Estado Atual, velocidade, posição GPS, é \gls{VEC}?, é \gls{OBU}?, etc)
...
3. Descarregamento de tarefas
    3.1 Modos de Execução
        ...
        3.1.2 Descarregamento \gls{V2I} (para \gls{RSU} via \gls{5G} \gls{NR})
        3.1.3 Descarregamento \gls{V2V} (para veículo vizinho via Sidelink)
    3.2
4. Coleta de métricas de desempenho do descarregamento de tarefas
    4.1 Métricas Globais
        ...
        4.1.6 Total de Tarefas descarregadas via \gls{V2I}
        4.1.7 Total de Tarefas descarregadas via \gls{V2V}
...
% Para validar a proposta de \textit{offloading} em um cenário veicular heterogêneo operando sob cobertura 5G NR e comunicação \gls{V2X} Sidelink, é fundamental estabelecer formalmente as entidades, as restrições físicas e os objetivos de otimização. Esta seção descreve o modelo matemático que fundamenta a arquitetura proposta.
...
% O sistema permite três modos de execução: processamento local, \textit{offloading} \gls{V2I} (para \gls{RSU} via 5G) e \textit{offloading} \gls{V2V} (para veículo vizinho via Sidelink).
...
% \end{equation}
% O consumo energético do veículo $i$ durante o \textit{offloading} considera a energia de transmissão de rádio:
% \begin{equation}
...
% Para garantir que veículos com \textit{hardware} inferior não sejam penalizados desproporcionalmente, utilizamos o Índice de Jain ($J$). Definindo a utilidade do veículo como o inverso do seu atraso percebido ($U_i = 1/T_i$), o índice é dado por:
% \begin{equation}
...
% \subsection{Formulação como Processo de Decisão de Markov (\gls{MDP})}

% Dada a natureza estocástica do canal sem fio e a mobilidade veicular, o problema de otimização estático é reformulado como um Processo de Decisão de Markov (\gls{MDP}), definido pela tupla $\mathcal{M} = <\mathcal{S}, \mathcal{A}, \mathcal{P}, \mathcal{R}, \gamma>$.
...
%     \end{equation}
%     onde $\mathbf{Q}_t$ representa as filas de tarefas, $\mathbf{H}_t$ a matriz de estado do canal (CSI) para links \gls{V2I} e \gls{V2V}, e $\mathbf{B}_t$ o status de bateria/recursos das OBUs.

%     \item \textbf{Espaço de Ações ($\mathcal{A}$):} O vetor de ações $a_t = [a_{1,t}, \dots, a_{V,t}]$ define o destino de \textit{offloading} para cada veículo, onde $a_{i,t} \in \{ \text{Local}, \text{RSU}, \text{Vizinhos} \}$.
...
% \begin{enumerate}
%     \item \textbf{Análise de Convergência:} Verificação da estabilidade da curva de recompensa média acumulada durante o treinamento do agente \gls{RL}, garantindo que o sistema converge para uma política estável.
...
% O módulo \texttt{toff} foi projetado de forma extensível, utilizando uma classe base abstrata chamada \texttt{OffloadingStrategy}. Esta classe define a interface de decisão que o agente \gls{DRL} deve implementar:
% \begin{center}
%     \texttt{virtual OffloadDecision Decide(Task, NeighborTable, DeviceSpecs);}
% \end{center}
% Neste contexto, o \textit{estado} do ambiente (\gls{MDP}) é composto pela \texttt{Task} (requisitos) e pela \texttt{NeighborTable} (recursos disponíveis). A \textit{ação} é a estrutura \texttt{OffloadDecision} (processar localmente ou descarregar para um vizinho VEC/V2V).

% O modelo híbrido proposto será implementado como uma nova estratégia (ex: \texttt{FuzzyDrlStrategy}) que herda de \texttt{OffloadingStrategy}. O módulo de Lógica \textit{Fuzzy} atuará como um pré-processador de estado, analisando a \texttt{NeighborTable} para gerar indicadores linguísticos (ex: "carga da rede", "qualidade do enlace"). O agente \gls{DRL} receberá esses indicadores e aprenderá a política ótima de decisão. As estratégias mais simples, como \texttt{LocalOnlyStrategy} e \texttt{RandomStrategy}, servirão como \textit{baselines} para a avaliação de desempenho.
...
% Ou então figuras podem ser incorporadas de arquivos externos, como é o caso da \autoref{fig-grafico-1}. Se a figura que ser incluída se tratar de um diagrama, um gráfico ou uma ilustração que você mesmo produza, priorize o uso de imagens vetoriais no formato PDF. Com isso, o tamanho do arquivo final do trabalho será menor, e as imagens terão uma apresentação melhor, principalmente quando impressas, uma vez que imagens vetorias são perfeitamente escaláveis para qualquer dimensão. Nesse caso, se for utilizar o Microsoft Excel para produzir gráficos, ou o Microsoft Word para produzir ilustrações, exporte-os como PDF e os incorpore ao documento conforme o exemplo abaixo. No entanto, para manter a coerência no uso de \textit{software} livre (já que você está usando LaTeX e abnTeX),  teste a ferramenta InkScape\index{InkScape}. ao CorelDraw\index{CorelDraw} ou ao Adobe Illustrator\index{Adobe! Illustrator}.  De todo modo, caso não seja possível  utilizar arquivos de imagens como PDF, utilize qualquer outro formato, como JPEG, GIF, BMP, etc.  Nesse caso, você pode tentar aprimorar as imagens incorporadas com o \textit{software} livre \index{Gimp}Gimp. Ele é uma alternativa livre ao Adobe Photoshop\index{Adobe! Photoshop}.
