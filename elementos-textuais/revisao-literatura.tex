...
A Tabela~\ref{tab:comparativo_lacunas} sintetiza as principais lacunas identificadas na literatura e como a proposta deste trabalho se posiciona para superá-las.

\begin{table}[ht!]
    \centering
    \Caption{\label{tab:comparativo_lacunas}Síntese das lacunas identificadas e a abordagem da proposta frente aos trabalhos relacionados}
    \renewcommand{\arraystretch}{1.5}
    \small
    \setlength{\tabcolsep}{6pt}
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3cm}|X|X|}
        \hline
        \textbf{Aspecto} & \textbf{Trabalhos Relacionados} & \textbf{Proposta (SAC-FTOPSIS)} \\ \hline
        Observabilidade & Assumem conhecimento perfeito e instantâneo dos recursos (Observabilidade Total). & Trata a incerteza e variabilidade via Lógica Fuzzy (Observabilidade Parcial). \\ \hline
        Canais de Comunicação & Consideram transmissões ideais, sem perdas ou atrasos estocásticos de canal. & Incorpora imperfeições de rede e dinâmica real de comunicação. \\ \hline
        Regime de Carga & Avaliados sob taxas de chegada de tarefas constantes ou estáticas. & Projetada para lidar com variações de regimes de demanda. \\ \hline
        Necessidade de Treinamento & Exigem grandes volumes de dados e fase extensiva de treinamento prévio (\gls{DRL}). & Hibridiza \gls{SAC} para aprendizado com \textit{Fuzzy-TOPSIS} para compressão de estado invariante à escala. \\ \hline
    \end{tabularx}
    \Fonte{Elaborado pelo autor}
\end{table}
