...
Os cenários são gerados sinteticamente a partir de variações de taxa de chegada de tarefas (\textit{tasks per second}, tps) e sementes de aleatoriedade distintas. O conjunto total compreende 661 cenários, distribuídos conforme a Tabela~\ref{tab:cenarios}.
...
\begin{tabular}{lrrr}
\toprule
\textbf{Política} & \textbf{Taxa deadline (\%)} & \textbf{JCT médio (s)} & \textbf{Energia média (J)} \\
\midrule
...
\gls{TOPSIS} como \textit{feature} + \textit{deadline} reward (S1 $\to$ T1) &
    $+4{,}3$ pp &
    Incorporar o escore $\Delta_{\text{obj}}$ e o JCT do \textit{rank}$_1$ no estado e substituir WSM pela recompensa \textit{deadline}-aware acrescenta contexto semântico moderado; o ganho é limitado pois o agente ainda precisa resolver a escolha \gls{RSU} vs.\ \gls{V2V} \\[4pt]
