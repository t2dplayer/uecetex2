...
O ambiente de treinamento e avaliação é implementado como um \gls{DES} desenvolvido em C++20 com co-rotinas, utilizando a biblioteca \texttt{simcpp20}~\cite{simcpp20}. Cada tarefa gerada pelos veículos é modelada como um processo concorrente que: (i)~aguarda o instante de chegada $t_j$; (ii)~consome o estado global do sistema para construir o vetor de observação; (iii)~executa a política de escalonamento; (iv)~atualiza as filas dos servidores; e (v)~registra as métricas reais de \gls{JCT} e energia ao término.

O servidor \gls{RSU} é modelado como um processador \gls{DVFS} de frequência nominal $F_{\text{RSU}} = 1{,}2\,\text{GHz}$, com fila compartilhada de capacidade $Q_{\max} = 100$ tarefas. Os veículos possuem frequências heterogêneas, amostradas no intervalo $[0{,}3;\,1{,}5]\,\text{GHz}$, conforme o arquivo \texttt{vehicle\_clocks.csv} de cada cenário. A comunicação \gls{V2I} utiliza largura de banda $B = 20\,\text{MHz}$, potência de transmissão $P_{\text{tx}} = 0{,}2\,\text{W}$ e modelo de perda de percurso conforme \gls{3GPP} TR 38.901 (UMi Street Canyon). As mesmas especificações valem para \gls{V2V}, com altura de antena de $1{,}5\,\text{m}$ e fator de ruído de $9\,\text{dB}$.

\subsection{Cenários de Teste e Treinamento}

Os cenários de treinamento são gerados utilizando o SUMO~\cite{lopez2018microscopic} para a mobilidade veicular e para carga de trabalho utilizou-se um \textit{trace} real de carga de trabalho do Alibaba~\cite{cheng2018characterizing}, realizando as conversões de escala de tempo para adequação ao cenário veicular. Já para o cenário de testes, foi utilizada a mesma mobilidade e foram criados 658 cenários com carga de trabalho segundo um processo de poisson, distribuídos conforme a Tabela~\ref{tab:cenarios}.
...
Cada cenário é representado por três arquivos: \texttt{workload\_distribuido.csv} (tarefas com $C_j$ em ciclos de \gls{CPU}, $D_j$ em segundos e posição geográfica), \texttt{vehicle\_clocks.csv} (frequência e coeficiente de energia de cada veículo) e \texttt{system\_params.csv} (parâmetros físicos fixos do ambiente). Os parâmetros físicos globais compartilhados por todos os cenários estão listados na Tabela~\ref{tab:system-params}.
...
O treinamento de cada política \gls{DRL} é conduzido em modo \textit{offline}, interagindo exclusivamente com o simulador, sem nenhum acesso ao ambiente real. O protocolo é o seguinte:
...
\midrule
$\alpha$ (custo \gls{JCT})    & Peso do \gls{JCT} na função objetivo     & $0{,}35$           \\
...
\begin{itemize}
    \item \textbf{Fase de Treinamento (Offline):} O algoritmo \gls{SAC} (Soft \textit{Actor}-\textit{Critic}) interage com um simulador estocástico para otimizar os pesos de uma rede neural Perceptron Multicamadas (\gls{MLP}). O ambiente simula a heterogeneidade dos servidores, a dinâmica autorregressiva (AR-1) das filas e as perdas de percurso realistas baseadas nos padrões \gls{3GPP} TR 38.901.
...
A Figura~\ref{fig:sac-topsis-arch} ilustra o fluxo completo de decisão da arquitetura \gls{SAC}-\gls{TOPSIS} em tempo de inferência.
...
\node[box, fill=gray!12, draw=gray!60!black] (tarefa) at (0,0)
    {\textbf{Tarefa} $j$\quad
     $C_j$ (ciclos CPU) $\cdot$ $D_j$ (deadline) $\cdot$ posição $\mathbf{x}_j$};
...
\node[box, fill=purple!12, draw=purple!60!black, minimum height=1.8cm,
    ...
    {\textbf{SAC Actor} --- Rede Neural (\gls{MLP})\\[4pt]
     ...
     treinado offline com replay buffer ($|\mathcal{B}|{=}50\,000$, batch$\,{=}\,1024$)};
...
\draw[->, thick, gray!55, dashed]
    (tarefa.west) -- ++(-1.5,0) |-
    node[rot, pos=0.3, left]{$C_j,\,D_j$ (bypass)} (estado.west);

%% Feedback de recompensa (treino offline)
\draw[->, thick, dashed, color=gray!55]
    (offload.east) -- ++(1.2,0) |-
    node[rot, pos=0.2, right]{recompensa $r_t$ (treino offline)} (mlp.east);

\end{tikzpicture}%
}}
\caption{Arquitetura \gls{SAC}-\gls{TOPSIS}: fluxo vertical de decisão. O módulo \gls{TOPSIS} comprime $N$ candidatos em vetor fixo $\mathbb{R}^6$; o \gls{SAC} \textit{Actor} decide binariamente entre execução local e \textit{offload} para o \textit{rank}$_1$. A seta tracejada cinza indica \textit{bypass} direto de $C_j$ e $D_j$; a tracejada direita mostra o loop de recompensa durante o treinamento \textit{offline}.}
...
O \gls{TOPSIS} avalia os $N$ nós candidatos por meio de três critérios, todos do tipo custo (minimizar), conforme a Tabela~\ref{tab:topsis-criterios}.
...
\begin{enumerate}
    ...
    \item $C_j/C_{\max}$: \textbf{complexidade da tarefa} — número de ciclos de \gls{CPU} normalizados, ancora o contexto computacional da decisão.
\end{enumerate}
...
\midrule
\gls{SAC}-Base   & 11D (bruto)     & 3 classes  & WSM           & --- \\
\gls{SAC}-A1     & 11D (bruto)     & binária    & WSM           & --- \\
\gls{SAC}-S1     & 6D (manual)     & 3 classes  & WSM           & --- \\
\gls{SAC}-R1     & 11D (bruto)     & 3 classes  & \textit{Deadline}-aware& --- \\
\gls{SAC}-T1     & 6D (\gls{TOPSIS})     & 3 classes  & \textit{Deadline}-aware& Feature \\
...
\begin{tabular}{lrrr}
\toprule
\textbf{Política} & \textbf{Taxa deadline (\%)} & \textbf{\gls{JCT} médio (s)} & \textbf{Energia média (J)} \\
\midrule
\gls{SAC}-Base       &  4{,}6 & 57{,}1 & 2422 \\
\gls{SAC}-A1         & 39{,}2 & 13{,}7 & 2106 \\
\gls{SAC}-S1         & 28{,}8 & 39{,}2 & 2028 \\
\gls{SAC}-R1         & 38{,}4 & 13{,}3 & 3153 \\
\gls{SAC}-T1         & 33{,}1 & 11{,}5 & 2612 \\
...
O espaço de ação é ternário $a_t \in \{0\!=\!\textsc{local},\, 1\!=\!\textsc{rsu},\, 2\!=\!\textsc{v2v}\}$: o agente deve aprender simultaneamente a \emph{decisão} de descarregar e a \emph{escolha do destino}. A recompensa usa o Método de Soma Ponderada (WSM) com pesos iguais sobre três critérios — \gls{JCT} estimado, energia total e ocupação de fila do executor escolhido — normalizados pelo valor máximo de cada coluna e mapeados para $[-1, 1]$:
\begin{equation}
...
\begin{equation}
    ...
    \begin{cases}
        ...
        \textsc{v2v}   & \text{se } a_t = 1,\; \text{\gls{RSU} inviável e \gls{V2V} viável} \\
        \textsc{local} & \text{se } a_t = 1 \text{ e nenhum offload viável (fallback)}
    \end{cases}
    \label{eq:acao-a1}
\end{equation}
O objetivo é isolar o efeito de reduzir o espaço de busca de 3 para 2 ações, sem nenhum critério inteligente de escolha de destino. A regra \gls{RSU}$\succ$\gls{V2V} é estática e ignora as condições dinâmicas de fila e distância no momento da decisão.
...
\end{itemize}
Essa variante permite quantificar separadamente quanto do ganho do \gls{SAC}-\gls{TOPSIS} provém do \gls{TOPSIS} como \emph{compressor de estado semântico} versus o \gls{TOPSIS} como \emph{prior de decisão de destino}.
...
\end{equation}
A soma $4{,}6 + 24{,}2 + 4{,}3 + 12{,}2 = 45{,}3\%$ é exata, confirmando que o caminho Base $\to$ S1 $\to$ T1 $\to$ \gls{TOPSIS} é perfeitamente aditivo. A Tabela~\ref{tab:ablacao-decomposicao} interpreta cada incremento.
...
\gls{TOPSIS} como \textit{feature} + \textit{deadline} reward (S1 $\to$ T1) &
    $+4{,}3$ pp &
    Incorporar o escore $\Delta_{\text{obj}}$ e o \gls{JCT} do \textit{rank}$_1$ no estado e substituir WSM pela recompensa \textit{deadline}-aware acrescenta contexto semântico moderado; o ganho é limitado pois o agente ainda precisa resolver a escolha \gls{RSU} vs.\ \gls{V2V} \\[4pt]
\gls{TOPSIS} como prior de decisão (T1 $\to$ \gls{SAC}-\gls{TOPSIS}) &
    $+12{,}2$ pp &
    Transferir a escolha de destino do agente para o \gls{TOPSIS} e reduzir o espaço de ação de 3 para 2 é o maior salto marginal: elimina a ambiguidade \gls{RSU}/\gls{V2V} e garante que o destino seja sempre o candidato de menor custo multicritério \\
\midrule
\textbf{Ganho total} & $\mathbf{+40{,}7}$ \textbf{pp} & \gls{SAC}-\gls{TOPSIS} vs.\ \gls{SAC}-Base \\
\bottomrule
\end{tabular}}
\end{table}

Para isolar o efeito da recompensa \textit{deadline-aware} sem alterar o estado, a variante \gls{SAC}-R1 atinge $38{,}4\%$ ($+33{,}8$ pp sobre o \textit{baseline} com estado 11D e ação ternária). Esse valor é maior que o de S1 ($28{,}8\%$), evidenciando que o sinal de recompensa assimétrico é individualmente mais poderoso que a compressão de estado quando aplicados em isolamento. Entretanto, no caminho sequencial, a maior parte do ganho de R1 é compartilhada com S1: o estado 6D já codifica urgência temporal em $\widehat{\text{JCT}}/D_i$, de modo que acrescentar a recompensa \textit{deadline}-aware sobre S1 produz um incremento marginal de apenas $+4{,}3$ pp (transição S1$\to$T1, que combina as duas mudanças).

\subsection{Análise dos Resultados}

Três achados principais emergem do estudo. Primeiro, a variante \gls{SAC}-A1 ($39{,}2\%$) demonstra que binarizar a ação \emph{sem} critério de seleção de destino já supera o \textit{baseline} em $+34{,}6$ pp — a simples redução do espaço de busca tem efeito expressivo — mas não alcança a proposta ($45{,}3\%$) porque a regra estática \gls{RSU}$\succ$\gls{V2V} ignora condições dinâmicas de fila e distância no momento da decisão. Segundo, a variante \gls{SAC}-T1 ($33{,}1\%$) mostra que usar o \gls{TOPSIS} apenas como \textit{feature} de estado contribui com $+4{,}3$ pp sobre S1, enquanto usar o \gls{TOPSIS} como \textit{prior} de decisão (\gls{SAC}-\gls{TOPSIS}) adiciona $+12{,}2$ pp sobre T1: a eficácia do \gls{TOPSIS} é quase três vezes maior quando ele assume o controle do destino do que quando fornece apenas informação semântica ao agente. Terceiro, o \gls{JCT} médio do \gls{SAC}-\gls{TOPSIS} ($6{,}6$ s) é $8{,}6\times$ menor que o do \gls{SAC}-Base ($57{,}1$ s), indicando que a combinação \gls{TOPSIS}$+$estado compacto$+$reward \textit{deadline}-aware produz não apenas mais tarefas concluídas no prazo, mas também latências substancialmente menores.
