...
To address this challenge, this paper proposes \textbf{SAC-TOPSIS}, a hybrid
...
Among all evaluated policies, SAC-TOPSIS achieves the highest compliance
...
\paragraph{Our prior work and the path to SAC-TOPSIS.}
...
Building on these insights, we now propose \textbf{SAC-TOPSIS}: a hybrid
...
Evaluated across 601~scenarios, SAC-TOPSIS achieves a deadline compliance rate
...
Section~\ref{sec:arch} describes the SAC-TOPSIS architecture in detail.
...
\subsection{Positioning of SAC-TOPSIS}
...
SAC-TOPSIS is, to our knowledge, the first architecture to use TOPSIS
...
%─────────────────────────────────────────────────────────────────────────────
\section{The SAC-TOPSIS Architecture}
\label{sec:arch}
%─────────────────────────────────────────────────────────────────────────────
...
\begin{figure}[t]
  ...
  \caption{SAC-TOPSIS architecture.
           TOPSIS compresses the variable-size candidate set into a fixed 6D
...
\begin{figure}[t]
  ...
  \caption{Performance comparison across key metrics.
           SAC-TOPSIS (dark blue) achieves the highest deadline compliance rate
           and SWQ index.
...
SAC-TOPSIS (ours) $\uparrow\mathbf{+39.9}$ & \textbf{44.36} & \textbf{0.1450} & 6.18 & 2337 \\
\midrule
...
Overall, SAC-TOPSIS achieves the highest deadline compliance ($44.36\%$)
...
SAC-TOPSIS consistently leads across all densities, maintaining $>34\%$
...
\begin{figure}[t]
  ...
  \caption{Deadline compliance rate as a function of vehicular density.
           ...
           SAC-TOPSIS (solid circles) consistently outperforms all baselines
...
SAC-TOPSIS exploits V2V offloading ($25.9\%$), which provides lower latency
...
\begin{figure}[t]
  ...
  \caption{Distribution of execution modes.
           SAC-TOPSIS adaptively balances local and V2V execution,
...
Figure~\ref{fig:training} compares the training curves of SAC-TOPSIS,
SAC-Base, TD3-TOPSIS, and PPO-TOPSIS.
All agents are trained for $400$ episodes in the $10$\,tasks/s scenario.
SAC-TOPSIS and TD3-TOPSIS exhibit faster reward improvement and less variance
...
\begin{figure}[t]
  ...
  \caption{Training convergence over 400 episodes.
           ...
           SAC-TOPSIS achieves stable convergence in both metrics.}
  \label{fig:training}
\end{figure}
...
(SAC-Base collapses, SAC-TOPSIS recovers), which confirms that the TOPSIS
...
To understand \emph{which} design choices within SAC-TOPSIS contribute most to
...
SAC-TOPSIS& Full (TOPSIS 6D + binary + deadline)& \textbf{44.36} & \textbf{+39.9}  \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{SAC-internal causal decomposition.
           ...
           The full SAC-TOPSIS combines all three components and achieves
...
3-class.
The full SAC-TOPSIS, which combines the TOPSIS state with the binary action
...
This paper presented \textbf{SAC-TOPSIS}, a hybrid architecture for
...
$\mathcal{V}(t)$ runs its own SAC-TOPSIS agent with decentralized coordination.
