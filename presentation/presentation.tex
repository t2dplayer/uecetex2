...
% ── Pacotes base ──────────────────────────────────────────────────────────────
\usepackage{siunitx,booktabs,xcolor,xspace,multirow,graphicx}
\graphicspath{{/home/sergiosvieira/Documentos/2025.2/qualifica/uecetex2/presentation/Template_LaTeX_Inetsys/figuras/}}
...
% ── Tema e cores (Template Inetsys/UECE) ──────────────────────────────────────
\usetheme{Madrid}
\usefonttheme{serif}
  ...
  \begin{beamercolorbox}[wd=\paperwidth,ht=1cm,sep=0.25cm]{frametitle}
    \begin{tikzpicture}[remember picture,overlay]
      \node[anchor=north east] at (current page.north east)
        {\includegraphics[height=0.75cm]{inetSys-fundo-azul-logo.png}};
    \end{tikzpicture}
...
\setbeamercolor{caption name}{fg=beamer_color}
...
% ── Metadados ────────────────────────────────────────────────────────────────
\title[\textbf{SAC-FTOPSIS para Redes Veiculares}]{%
  \textbf{Modelo de Decisão Baseado em Lógica \textit{Fuzzy}-TOPSIS e\\
  Aprendizado por Reforço Profundo\\
  para o \textit{Offloading} de Tarefas\\
  em Redes Veiculares}}
\author[\textbf{Antônio Sérgio de Sousa Vieira}]{%
  \textbf{Antônio Sérgio de Sousa Vieira}}
\institute{%
  \small\textbf{Doutorado em Ciência da Computação --- Redes de Computadores} \\[4pt]
  Orientador: Prof. Dr. Joaquim Celestino Júnior (UECE/CCT) \\
  Co-orientador: Prof. Dr. Alisson Barbosa de Souza}
\date{\textbf{Fortaleza -- Ceará, 2026}}

% ── Capa (template Inetsys) ───────────────────────────────────────────────────
\renewcommand{\maketitle}{%
  \begin{center}
    \includegraphics[height=1.6cm]{uece_inetsys_desc_logo.png}
  \end{center}
  \titlepage
}
...
\begin{tikzpicture}[>=Stealth, thick, font=\tiny\ttfamily,
  ...
  \node[black, font=\tiny\ttfamily, align=center] at (-4.0, -1.0)
    {REPLAY\\BUFFER};
...
% 24b-0. ARTIGO DE REFERÊNCIA: FOFF
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain]
\begin{center}
\includegraphics[page=1, width=\textwidth, height=0.92\textheight, keepaspectratio,trim={0 14cm 0 0}, clip]{/home/sergiosvieira/Documentos/2025.2/qualifica/leitura-obrigatoria/FOFF_an_energy-efficient_task_offloading_in_VEC-en.pdf}
\end{center}
\end{frame}
}

% 24b-2. ORIGINALIDADE: CONTRIBUIÇÃO CONCEITUAL
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain, t]
\color{black}
\vspace{0.20cm}
\hspace{0.3cm}{\large\bfseries\ttfamily FOFF vs.\ PROPOSTA: DIFERENÇAS FUNDAMENTAIS}
\vspace{0.18cm}

\hspace{0.3cm}\begin{minipage}{0.93\textwidth}
{\scriptsize\ttfamily

\begin{tabular}{@{}p{2.8cm}|p{3.8cm}|p{4.2cm}@{}}
\textbf{Dimensão}        & \textbf{FOFF [Vieira et al., 2025]}              & \textbf{Proposta (SAC+Fuzzy-TOPSIS)}                 \\
\hline\noalign{\vspace{3pt}}
Decisão de offload       & Fuzzy logic (regras estáticas)                   & SAC --- política estocástica aprendida          \\[3pt]
\hline
Papel do TOPSIS          & Ranqueia candidatos para seleção                 & Comprime estado variável ($N$) em vetor 6D fixo \\[3pt]
\hline
Adaptação ao ambiente    & Não se adapta após implantação                   & Aprende continuamente via replay buffer         \\[3pt]
\hline
Robustez à mobilidade    & Degradação sob alta densidade variável (visão míope)           & Invariante à escala por design                  \\[3pt]
\hline
Objetivo principal       & Deadline + \textbf{Eficiência energética}                            & \textbf{Deadline} + Eficiência energética\\
\hline
\end{tabular}

\vspace{6pt}
$\Rightarrow$ O TOPSIS sai do papel de \textit{seletor} (FOFF) para \textit{compressor de estado}
(proposta), habilitando um agente DRL com entrada de dimensão fixa e política adaptativa.
}
\end{minipage}
\end{frame}
}

% 24b-1. ARTIGO DE REFERÊNCIA: MIRIYALA
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain]
\begin{center}
\includegraphics[page=1, width=\textwidth, height=0.92\textheight, keepaspectratio,trim={0 12cm 0 0}, clip]{/home/sergiosvieira/Documentos/2025.2/qualifica/uecetex2/related-papers/section-related-works/literature-review/Deep learning approaches for computation offloading in edge.pdf}
\end{center}
\end{frame}
}


% Slide A: Lacunas 1–5 (atendidas)
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain, t]
\color{black}
\vspace{0.15cm}
\hspace{0.3cm}{\large\bfseries\ttfamily LACUNAS ATENDIDAS PELA PROPOSTA [Miriyala \& Chirra, 2026]}
\vspace{0.12cm}
...
\textbf{\#} & \textbf{Lacuna (Tabela 9, p.\,20)} & \textbf{Como a proposta responde} & \textbf{Status} \\
\hline\noalign{\vspace{3pt}}
1 & \textit{Flat vectors} ignoram topologia variável ($N$ nós) e relações estruturais entre dispositivos &
  TOPSIS comprime $N$ candidatos em vetor 6D fixo --- invariante à escala &
  \textbf{Sim} \\[4pt]
2 & Políticas estáticas colapsam sob não-estacionariedade e alta mobilidade &
  SAC com entropia máxima + replay buffer: adaptação contínua sem retreino completo &
  \textbf{Sim} \\[4pt]
3 & Vulnerabilidade a perturbações adversariais nas observações de estado &
  Estado 6D normalizado via Fuzzy-TOPSIS atenua sensibilidade a outliers e ruído de medição &
  \textbf{Parcial} \\[4pt]
4 & Modelos leves para borda extrema (\textless50\,MB, \textless100\,MFLOPS) &
  MLP $6{\to}64{\to}64{\to}2$: $\approx$8\,KB de pesos; inferência em microssegundos &
  \textbf{Sim} \\[4pt]
5 & \textit{Continual learning} sem retreino ao mudar distribuição do ambiente &
  SAC \textit{off-policy} + replay buffer: atualização incremental sem catastrophic forgetting &
  \textbf{Sim} \\
\end{tabular}
...
% Slide B: Lacunas 6–9 (futuro / fora do escopo)
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain, t]
\color{black}
\vspace{0.15cm}
\hspace{0.3cm}{\large\bfseries\ttfamily LACUNAS NÃO COBERTAS --- DIREÇÕES FUTURAS [Miriyala \& Chirra, 2026]}
\vspace{0.20cm}

\hspace{0.3cm}\begin{minipage}{0.93\textwidth}
{\scriptsize\ttfamily

\begin{tabular}{@{}c|p{4.0cm}|p{4.6cm}|c@{}}
\textbf{\#} & \textbf{Lacuna (Tabela 9, p.\,20)} & \textbf{Posição da Proposta} & \textbf{Status} \\
\hline\noalign{\vspace{3pt}}
6 & Benchmarks padronizados (ETSI MEC) para comparação reprodutível &
  Simulador próprio; migração ao NS-3 (módulo 5G-LENA) planejada &
  Próximo Passo \\[4pt]
7 & Otimização quântica híbrida (quantum-classical DRL) &
  Fora do escopo desta proposta &
  Não \\[4pt]
8 & Comunicação semântica integrada à decisão de offload &
  Fora do escopo desta proposta &
  Não \\[4pt]
9 & Digital Twin para treino seguro e transferência Sim-to-Real &
  Validação de alta fidelidade via NS-3 planejada como próximo passo &
  Próximo Passo \\
\end{tabular}

\vspace{8pt}
$\Rightarrow$ Atualmente a proposta cobre \textbf{4 de 9 lacunas completamente} e 1 parcialmente, o próximo passo será integrar a lógica Fuzzy ao TOPSIS e atender as lacunas 6 e 7.
}
\end{minipage}
\end{frame}
}
...
DE NÓS CANDIDATOS MUDA A TODO INSTANTE.
...
% 25b. FORMULAÇÃO DO PROBLEMA: FUNÇÃO OBJETIVO
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain, t]
\color{black}
...
\vspace{0.30cm}
\hspace{0.3cm}{\Large\bfseries\ttfamily FORMULAÇÃO DO PROBLEMA}

\vspace{0.20cm}
\hspace{0.3cm}
\begin{minipage}{0.92\textwidth}
{\scriptsize\ttfamily\setlength{\baselineskip}{1.5\baselineskip}
O PROBLEMA É FORMULADO COMO UM POMDP. O OBJETIVO CENTRAL É
MAXIMIZAR A TAXA DE TAREFAS CONCLUÍDAS DENTRO DO PRAZO,
PONDERADA COM O CUSTO NORMALIZADO DE LATÊNCIA E ENERGIA.
}
\end{minipage}
...
$\displaystyle
\max_{\pi}\; J(\pi) \;=\;
\underbrace{w_s \cdot \mathbb{E}_\pi\!\left[\mathbf{1}(\text{JCT}_i \leq D_i)\right]}_{\text{taxa de sucesso}}
\;-\;
\underbrace{w_c \cdot \mathbb{E}_\pi\!\left[\frac{1}{2}\!\left(\frac{\text{JCT}_i}{\text{JCT}^{\max}_i} + \frac{E_i}{E^{\text{ref}}_i}\right)\right]}_{\text{custo normalizado} \;\in\;[0,1]}
$
\vspace{4pt}

{\scriptsize\ttfamily $w_s = 0{,}60 > w_c = 0{,}40$}
\vspace{2pt}
\end{block}
...
$\mathbf{1}(\text{JCT}_i \leq D_i)$ & Indicador de sucesso: 1 se a tarefa $i$ concluiu dentro do prazo $D_i$ \\[3pt]
$\text{JCT}_i / \text{JCT}^{\max}_i$ & Latência normalizada $\in [0,1]$ (pior caso por tarefa) \\[3pt]
$E_i / E^{\text{ref}}_i$ & Energia normalizada $\in [0,1]$ (pior caso por tarefa) \\[3pt]
$w_s > w_c$ & Violar prazos em segurança veicular é mais grave do que custo energético \\
\end{tabular}
}
...
\vspace{0.15cm}
\hspace{0.3cm}{\large\bfseries\ttfamily RECOMPENSA \textit{DEADLINE}-AWARE}
...
\begin{tikzpicture}[>=Stealth]
  ...
  % rbox com anchor=north em (0,0): topo da caixa em y=0, cresce para baixo
  ...
  % E^max — abaixo da caixa
  \node[font=\small\ttfamily, align=center, text width=12cm, below=0.30cm of rbox] (eref)
    {$E^{\max}_i = \max\!\bigl(E^{\ell}_i,\; E^{\text{rsu}}_i\cdot\mathbf{1}_{\text{viável}},\; E^{\text{v2v}}_i\cdot\mathbf{1}_{\text{viável}}\bigr)$ — energia máxima entre os modos viáveis (por tarefa, por instante)};

  % Eixo — abaixo do eref
  \node[below=0.50cm of eref] (axisanchor) {};
  \draw[black, line width=1.2pt, ->] (-6.2,-5.80) -- (6.8,-5.80)
    node[right, font=\small\ttfamily]{$r_i$};

  % FALHA [-2,-1]
  \fill[black!22] (-6.0,-6.20) rectangle (-2.8,-5.80);
  \node[font=\small\bfseries\ttfamily] at (-4.4,-6.02) {FALHA $[\,-2{,}0;\,-1{,}0\,]$};
  \draw[black, line width=1.4pt] (-6.0,-6.30) -- (-6.0,-5.70);
  \draw[black, line width=1.4pt] (-2.8,-6.30) -- (-2.8,-5.70);

  % GAP
  \draw[black!50, <->, line width=1.0pt] (-2.8,-6.50) -- (-1.2,-6.50)
    node[midway, below, yshift=-8pt, font=\small\ttfamily]{gap $= 1{,}5$};
  \draw[black, line width=1.4pt] (-1.2,-6.30) -- (-1.2,-5.70);

  % SUCESSO [0.5,1]
  \fill[black!65] (-1.2,-6.20) rectangle (4.2,-5.80);
  \node[font=\small\bfseries\ttfamily, text=white] at (1.5,-6.02) {SUCESSO $[\,0{,}5;\,1{,}0\,]$};
  \draw[black, line width=1.4pt] (4.2,-6.30) -- (4.2,-5.70);

  % Labels
  \node[font=\small\ttfamily] at (-6.0,-6.70) {$-2{,}0$};
  \node[font=\small\ttfamily] at (-2.8,-6.70) {$-1{,}0$};
  \node[font=\small\ttfamily] at (-1.2,-6.70) {$0{,}5$};
  \node[font=\small\ttfamily] at ( 4.2,-6.70) {$1{,}0$};

  % Anotações
  \node[font=\small\ttfamily, align=center, text width=4.5cm] at (-4.4,-7.60)
    {penalidade proporcional ao\\atraso relativo $(\widehat{\text{JCT}}-D_i)/D_i$};
  \node[font=\small\ttfamily, align=center, text width=4.5cm] at (1.5,-7.60)
    {recompensa decresce com\\latência e energia normalizadas};
  \node[font=\small\ttfamily, align=center, text width=3.8cm] at (5.8,-7.60)
    {cumprir prazo supera\\qualquer eficiência};
...
\begin{tikzpicture}[>=Stealth]
  ...
  \node[font=\tiny\ttfamily, align=center] at (3.00, -1.62)
    {$\Delta_{\text{obj}} = \bigl(\widehat{\text{JCT}}_\ell - \widehat{\text{JCT}}_{r_1}\bigr)/D_i$\\[2pt]
     $>0$: offload mais rápido\quad $<0$: local mais rápido};
...
\vspace{0.15cm}
\hspace{0.3cm}{\large\bfseries\ttfamily ALGORITMO 1 — TOPSIS (RANQUEAMENTO MULTICRITÉRIO)}
...
\Input{Matriz de decisão $\mathbf{X} \in \mathbb{R}^{m \times n}$; vetor de pesos $\mathbf{w}$}
\Output{Vetor de proximidade $\mathbf{C}^*$ (alternativas ranqueadas)}
\tcp{Passos 1--2: Normalização $L_2$ por coluna e ponderação}
\For{$j = 1$ \KwTo $n$}{
  \For{$i = 1$ \KwTo $m$}{
    $r_{ij} \gets x_{ij} \big/ \sqrt{\sum_{k=1}^{m} x_{kj}^2}$\;
    $v_{ij} \gets w_j \cdot r_{ij}$\;
  }
  \tcp{Passo 3: Soluções ideal positiva $A^+$ e negativa $A^-$}
  \eIf{critério $j$ é de custo}{
    $A^+_j \gets \min_i v_{ij}$;\quad $A^-_j \gets \max_i v_{ij}$\;
  }{
    $A^+_j \gets \max_i v_{ij}$;\quad $A^-_j \gets \min_i v_{ij}$\;
  }
}
\tcp{Passos 4--5: Distâncias $L_2$ (Euclidianas) e coeficiente de proximidade}
\For{$i = 1$ \KwTo $m$}{
  $d^+_i \gets \sqrt{\sum_{j=1}^{n}(v_{ij}-A^+_j)^2}$;\quad
  $d^-_i \gets \sqrt{\sum_{j=1}^{n}(v_{ij}-A^-_j)^2}$\;
  $C^*_i \gets d^-_i\,/\,(d^+_i + d^-_i)$\;
}
\Return Ordenação decrescente de $\mathbf{C}^*$\;
\end{algorithm}
\end{minipage}}
...
% 26d-geo. TOPSIS: REPRESENTAÇÃO GEOMÉTRICA
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain, t]
\color{black}
...
\vspace{0.20cm}
\hspace{0.3cm}{\large\bfseries\ttfamily TOPSIS: REPRESENTAÇÃO GEOMÉTRICA DO RANQUEAMENTO}
\vspace{0.10cm}
...
\vspace{0.15cm}
\hspace{0.3cm}\begin{minipage}{0.90\textwidth}
{\scriptsize\ttfamily\setlength{\baselineskip}{1.6\baselineskip}
$A_1$ É PREFERÍVEL A $A_3$ PORQUE $d^+_1 < d^+_3$ (MAIS PRÓXIMA DO IDEAL POSITIVO)
E $d^-_1 > d^-_3$ (MAIS DISTANTE DO IDEAL NEGATIVO). O ESCORE
$C^*_i = d^-_i\,/\,(d^+_i + d^-_i) \in [0,1]$ CAPTURA AMBAS AS CONDIÇÕES SIMULTANEAMENTE.
}
\end{minipage}
...
% 26. A PROPOSTA: PIPELINE SAC-FTOPSIS (MELHORADO)
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain, t]
\color{black}
...
\vspace{0.35cm}
\hspace{0.3cm}{\Large\bfseries\ttfamily PRIMEIRO PASSO DA PROPOSTA: PIPELINE SAC-FTOPSIS}
...
% 26e. ALGORITMO 2: SAC-FTOPSIS (INFERÊNCIA)
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain, t]
\color{black}
...
\vspace{0.15cm}
\hspace{0.3cm}{\large\bfseries\ttfamily ALGORITMO 2 — SAC-FTOPSIS (INFERÊNCIA)}
...
\SetKwInOut{Input}{Entrada}
\SetKwInOut{Output}{Saída}
\Input{Tarefa $i$: $C_i$, $D_i$, $\mathbf{x}_i$;\; $N$ candidatos $\mathcal{M}$;\; MLP $\pi_\theta$;\; $Q_{\ell}$, $Q_{\mathrm{r1}}$, $Q_{\max}$, $C_{\max}$}
\Output{Ação $a_t \in \{0,1\}$: local ou offload para rank$_1$}
\tcp{Passo 1 — Matriz de decisão ($N \times 3$)}
\For{$k = 1$ \KwTo $N$}{
  $x_{k1} \gets \widehat{\mathrm{JCT}}(m_k)$;\quad
  $x_{k2} \gets E(m_k)$;\quad
  $x_{k3} \gets \|\mathbf{x}_i - \mathbf{x}_{m_k}\|$\;
}
\tcp{Passo 2 — Ranqueamento via Algoritmo 1 (todos critérios de custo)}
$\mathbf{w} \gets [0{,}40;\;0{,}30;\;0{,}30]$;\quad
$\mathbf{C}^* \gets \text{Algoritmo\,1}(\mathbf{X},\mathbf{w})$;\quad
...
\eIf{$a_t = 0$}{\Return \textsc{local}}{\Return \textsc{offload} para $\mathrm{rank}_1$}
\end{algorithm}
\end{minipage}}
...
\textbf{SAC-FTOPSIS} & \textbf{6D (TOPSIS)} & \textbf{binária} & \textbf{Deadline-aware} & \textbf{Decisão} \\
\hline
\end{tabular}}
...
\textbf{SAC-FTOPSIS} & \textbf{45,29} & \textbf{6,6} & 2582 & $\mathbf{+40,7}$ \\
\hline
\end{tabular}}
...
% 27c. ESTUDO DE ABLAÇÃO: ANÁLISE DOS ACHADOS
{
\setbeamercolor{background canvas}{bg=white}
\begin{frame}[plain, t]
\color{black}
...
\vspace{0.35cm}
\hspace{0.3cm}{\Large\bfseries\ttfamily ESTUDO DE ABLAÇÃO: ANÁLISE DOS ACHADOS}
...
\begin{tikzpicture}[>=Stealth]
  ...
  % Achado 1
  \node[draw=black, line width=0.8pt, fill=black!04, rounded corners=4pt,
        inner sep=7pt, font=\small, align=left, text width=11cm] (a1) at (0, 0) {
    \textbf{\ttfamily ACHADO 1 — AÇÃO BINÁRIA ESTÁTICA JÁ SUPERA O BASELINE}\\[3pt]
    {\scriptsize\ttfamily
    SAC-A1 (11D, binária, regra VEC$\succ$V2V): \textbf{39,2\%} $\to$ $+34,6$ pp sobre SAC-Base (4,6\%).\\
    SIMPLES REDUÇÃO DO ESPAÇO DE BUSCA TEM EFEITO EXPRESSIVO.\\
    PORÉM NÃO ALCANÇA SAC-FTOPSIS (45,29\%): A REGRA ESTÁTICA IGNORA\\
    CONDIÇÕES DINÂMICAS DE FILA E DISTÂNCIA NO MOMENTO DA DECISÃO.}
  };

  % Achado 2
  \node[draw=black, line width=0.8pt, fill=black!04, rounded corners=4pt,
        inner sep=7pt, font=\small, align=left, text width=11cm,
        below=0.25cm of a1] (a2) {
    \textbf{\ttfamily ACHADO 2 — TOPSIS COMO DECISÃO VALE 3$\times$ MAIS QUE COMO FEATURE}\\[3pt]
    {\scriptsize\ttfamily
    SAC-T1 (TOPSIS como feature): $+4,3$ pp sobre SAC-S1.\\
    SAC-FTOPSIS (TOPSIS como prior de destino): $+12,2$ pp sobre SAC-T1.\\
    EFICÁCIA DO TOPSIS É QUASE 3$\times$ MAIOR QUANDO ELE ASSUME O CONTROLE\\
    DO DESTINO DO QUE QUANDO APENAS INFORMA O ESTADO DO AGENTE.}
  };

  % Achado 3
  \node[draw=black, line width=1.4pt, fill=black!10, rounded corners=4pt,
        inner sep=7pt, font=\small, align=left, text width=11cm,
        below=0.25cm of a2] (a3) {
    \textbf{\ttfamily ACHADO 3 — GANHO EM LATÊNCIA: 8,6$\times$ MENOR}\\[3pt]
    {\scriptsize\ttfamily
    JCT MÉDIO SAC-FTOPSIS: \textbf{6,6 s} $\times$ SAC-BASE: \textbf{57,1 s}.\\
    A COMBINAÇÃO TOPSIS + ESTADO COMPACTO + REWARD DEADLINE-AWARE\\
    PRODUZ NÃO SÓ MAIS TAREFAS NO PRAZO, MAS LATÊNCIAS SUBSTANCIALMENTE MENORES.\\[3pt]
...
\hline
 1 & \textbf{sac-topsis}$^\star$ & \textbf{45,29} & \textbf{0,1484} &  6,5 & 25,8 & DRL+TOPSIS \\
 2 & ppo-topsis$^\star$          & 42,75 & 0,1426 &  5,9 & 21,3 & DRL+TOPSIS \\
 3 & ppo-base                    & 42,42 & 0,1418 &  6,0 & 24,4 & DRL-Base   \\
 4 & ppo-topsis-compact$^\star$  & 42,42 & 0,1418 &  6,0 & 24,4 & DRL+TOPSIS \\
 5 & td3-base                    & 42,42 & 0,1418 &  6,0 & 24,4 & DRL-Base   \\
 6 & a2c-topsis$^\star$          & 42,42 & 0,1418 &  6,0 & 24,4 & DRL+TOPSIS \\
 7 & td3-topsis$^\star$          & 44,00 & 0,1388 &  3,7 & 23,0 & DRL+TOPSIS \\
 8 & sac-topsis-9d$^\star$       & 42,91 & 0,1386 &  5,9 & 21,0 & DRL+TOPSIS \\
 9 & sac-topsis-manhattan$^\star$& 41,75 & 0,1385 & 11,8 & 23,7 & DRL+TOPSIS \\
10 & sac-t1$^\star$              & 33,06 & 0,1270 & 11,5 & 26,1 & DRL+TOPSIS \\
11 & greedy-unfair               & 41,38 & 0,1262 &  5,6 &  7,4 & Heurística\\
12 & sac-a1                      & 39,21 & 0,1256 & 13,7 & 21,1 & DRL-Base   \\
\hline
\end{tabular}}
...
\begin{minipage}{0.92\textwidth}
{\scriptsize\ttfamily\setlength{\baselineskip}{1.5\baselineskip}
O SAC-FTOPSIS LIDERA COM SWQ 0,1484. OS ALGORITMOS PPO, TD3 E A2C
CONVERGEM PARA 100\% LOCAL (SWQ $\approx$ 0,1418). O GREEDY-UNFAIR COM
CONHECIMENTO PERFEITO DO ESTADO DA SIMULAÇÃO FICA ABAIXO DO SAC-FTOPSIS (0,1262).
}
\end{minipage}
...
\textbf{\#} & \textbf{POLÍTICA} & \textbf{TAXA\%} & \textbf{SWQ} & \textbf{JCT(s)} & \textbf{E(J)} & \textbf{CATEG.} \\
\hline
13 & sac-r1        & 38,36 & 0,1223 & 13,4 & 31,4 & DRL-Base   \\
14 & sac-s1        & 28,81 & 0,1122 & 39,3 & 20,3 & DRL-Base   \\
15 & gini          & 35,34 & 0,1057 &  3,5 & 39,4 & Heurística\\
16 & dqn-topsis$^\star$ & 26,02 & 0,0997 & 21,7 & 24,0 & DRL+TOPSIS \\
...
21 & a2c-base      & 21,85 & 0,0772 & 13,7 & 48,9 & DRL-Base   \\
22 & dqn-base      & 22,15 & 0,0686 & 13,6 & 44,4 & DRL-Base   \\
23 & greedy-fair   & 20,65 & 0,0528 & 40,8 &  5,6 & Heurística\\
24 & sac-base      &  4,58 & 0,0119 & 57,1 & 24,2 & DRL-Base   \\
...
\begin{minipage}{0.92\textwidth}
{\scriptsize\ttfamily\setlength{\baselineskip}{1.5\baselineskip}
O SAC-BASE (4,58\%) COLAPSA POR USAR ESTADO 11D BRUTO SEM TOPSIS:
...
% E $w_c{=}0{,}40$ PARA CUSTO NORMALIZADO (MIN-MAX) DE LATÊNCIA E ENERGIA.
% AVALIADO EM 658 CENÁRIOS V2X, O SAC-FTOPSIS ATINGIU $J(\pi){=}0{,}2454$.
% }
% \end{minipage}
...
%       \;-\;
%       \underbrace{w_c \cdot \mathbb{E}_\pi\!\left[\frac{1}{2}\!\left(\frac{\text{JCT}_i}{\text{JCT}^{\max}_i} + \frac{E_i}{E^{\max}_i}\right)\right]}_{\text{custo norm.\ min-max}\;\in\,[0,1]\;(w_c = 0{,}40)}$};

%   % Parâmetros — abaixo da caixa com folga
%   \node[font=\scriptsize\ttfamily, align=center, text width=14cm, below=0.15cm of eqbox] (params) {%
%     $\text{JCT}^{\max}_i = \max\!\bigl(\widehat{\text{JCT}}^{\ell}_i,\; \widehat{\text{JCT}}^{\text{rsu}}_i\cdot\mathbf{1}_{\text{viável}},\; \widehat{\text{JCT}}^{\text{v2v}}_i\cdot\mathbf{1}_{\text{viável}}\bigr)$\\[2pt]
%     $E^{\max}_i = \max\!\bigl(E^{\ell}_i,\; E^{\text{rsu}}_i\cdot\mathbf{1}_{\text{viável}},\; E^{\text{v2v}}_i\cdot\mathbf{1}_{\text{viável}}\bigr)$\quad
%     normalização min-max por tarefa — garante custo $\in[0,1]$ para qualquer JCT};

%   % Top 3 ranking de J(π)
%   \node[font=\scriptsize\bfseries\ttfamily, below=0.15cm of params] (top3label) {TOP 3 — FUNÇÃO OBJETIVO $J(\pi)$};

%   \node[draw=black, line width=1.6pt, fill=black!16, rounded corners=3pt,
%         font=\scriptsize\bfseries\ttfamily, align=center,
%         minimum width=3.2cm, minimum height=0.60cm] at (-4.2,-2.90)
%     {1º SAC-FTOPSIS\\$J{=}0{,}2454$};

%   \node[draw=black, line width=0.9pt, fill=black!08, rounded corners=3pt,
%         font=\scriptsize\ttfamily, align=center,
%         minimum width=3.0cm, minimum height=0.58cm] at (0.0,-2.90)
%     {2º PPO-TOPSIS\\$J{=}0{,}2344$};

%   \node[draw=black, line width=0.9pt, fill=black!05, rounded corners=3pt,
%         font=\scriptsize\ttfamily, align=center,
%         minimum width=3.0cm, minimum height=0.58cm] at (4.0,-2.90)
%     {3º PPO-Base\\$J{=}0{,}2295$};

%   % Barras de J(π): escala * 30
%   \draw[black, line width=0.6pt] (-6.5,-3.65) -- (1.5,-3.65);
%   \fill[black!80] (-6.5,-4.00) rectangle (-6.5+7.36,-3.70);
%   \fill[black!35] (-6.5,-4.32) rectangle (-6.5+7.03,-4.02);
%   \fill[black!20] (-6.5,-4.64) rectangle (-6.5+6.89,-4.34);
%   \node[font=\scriptsize\ttfamily, anchor=west] at (0.90,-3.85) {SAC-FTOPSIS 0,2454};
%   \node[font=\scriptsize\ttfamily, anchor=west] at (0.90,-4.17) {PPO-TOPSIS 0,2344};
%   \node[font=\scriptsize\ttfamily, anchor=west] at (0.90,-4.49) {PPO-Base 0,2295};
%   \node[font=\scriptsize\ttfamily] at (-2.5,-4.90) {$J(\pi)$ (FUNÇÃO OBJETIVO)};
...
REDUZIDO EM 44,5\%. AO SAC: $+$1147\% (4,58\% $\to$ 45,29\%).\\
COMPARAÇÃO MAIS JUSTA: SAC-T1 (TOPSIS COMO FEATURE, 3 CLASSES) $\to$
SAC-FTOPSIS (TOPSIS COMO DECISÃO, BINÁRIA): $+$12,3 PP, $+$16,8\% SWQ.
...
\begin{tikzpicture}[>=Stealth,
  ...
  % SAC-T1 → SAC-FTOPSIS (comparação justa: mesmo TOPSIS, só muda ação)
  \node[font=\tiny\ttfamily, anchor=east] at (-0.25,-0.65) {\textit{SAC-T1}};
  \fill[black!30] (0,-0.92) rectangle (5.27,-0.48);   % SAC-T1: 32.96%
  \fill[black!72] (5.31,-0.92) rectangle (7.22,-0.48); % SAC-FTOPSIS: 45.21%
...
% \vspace{0.35cm}
% \hspace{0.3cm}{\Large\bfseries\ttfamily A ASSIMETRIA: POR QUE O TOPSIS NÃO AJUDOU O PPO?}

% \vspace{0.15cm}
% \hspace{0.3cm}
% \begin{minipage}{0.92\textwidth}
% {\scriptsize\ttfamily\setlength{\baselineskip}{1.5\baselineskip}
% EXPERIMENTO CONTROLADO: PPO COM ESTADO 6D, AÇÃO BINÁRIA E MESMA RECOMPENSA DO SAC-FTOPSIS.
% RESULTADO: PPO-TOPSIS-COMPACT COLAPSA PARA LOCAL (42,42\% = BASELINE LOCAL = PPO-BASE).
% O TOPSIS NÃO AGREGA VALOR AO PPO: O AGENTE ON-POLICY APRENDE A IGNORAR O ESTADO E ESCOLHER
% SEMPRE LOCAL. A DIFERENÇA SAC $-$ PPO (45,29\% $-$ 42,42\% $=$ 2,87\,pp) É ATRIBUÍVEL
% EXCLUSIVAMENTE AO ALGORITMO: OFF-POLICY + ENTROPIA PREVINEM O COLAPSO.
% }
% \end{minipage}
...
%         minimum height=0.55cm] at (-3.0,-3.55) {45,29\% \ DEADLINE};
%   \node[draw=black, line width=0.8pt, fill=black!06, rounded corners=3pt,
%         font=\scriptsize\ttfamily, minimum width=3.6cm,
%         minimum height=0.55cm] at ( 3.0,-3.55) {42,42\% \ DEADLINE};
%   \draw[black,->,line width=1.6pt] (0.4,-3.55) -- (-1.4,-3.55)
%     node[midway, above, font=\tiny\bfseries\ttfamily] {$\Delta{=}2{,}87$\,pp};
...
\begin{tikzpicture}[>=Stealth, thick,
  ...
  \node[etapa, fill=black!12] (b1) at (-4.2, 0)
    {SAC-FTOPSIS\\(QUALIFICAÇÃO)\\{\tiny $\checkmark$ 658 CENÁRIOS}};
