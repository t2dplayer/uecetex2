...
\begin{tikzpicture}[>=Stealth]
  ...
  \node[draw=black, line width=0.8pt, fill=black!04, rounded corners=4pt,
    ...
    PORÉM NÃO ALCANÇA SAC-TOPSIS (45,3\%): A REGRA ESTÁTICA IGNORA\\
...
\begin{tikzpicture}[>=Stealth]
  ...
  \node[draw=black, line width=1.4pt, fill=black!06, rounded corners=4pt,
    ...
    $\displaystyle J(\pi) \;=\;
      ...
      \;-\;
      \underbrace{w_c \cdot \mathbb{E}_\pi\!\left[\frac{1}{2}\!\left(\frac{\text{JCT}_i}{\text{JCT}^{\max}_i} + \frac{E_i}{E^{\text{ref}}_i}\right)\right]}_{\text{custo norm.\ min-max}\;\in\,[0,1]\;(w_c = 0{,}40)}$};

  % Parâmetros — abaixo da caixa com folga
  \node[font=\tiny\ttfamily, align=center, below=0.20cm of eqbox]
    {$\text{JCT}^{\max}_i = \max\!\bigl(\widehat{\text{JCT}}^{\ell}_i,\, \widehat{\text{JCT}}^{\text{rsu}}_i,\, \widehat{\text{JCT}}^{\text{v2v}}_i\bigr)$,\quad
     $E^{\text{ref}}_i = \max\!\bigl(E^{\ell}_i,\, E^{\text{rsu}}_i,\, E^{\text{v2v}}_i\bigr)$: normalização min-max por tarefa --- garante custo $\in[0,1]$ para qualquer JCT};
