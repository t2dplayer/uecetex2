...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[white, anchor=north, font=\footnotesize\bfseries]
    at ([yshift=-0.40cm]current page.north)
    {UNIVERSIDADE ESTADUAL DO CEARÁ --- UECE};
  \node[white, anchor=north, font=\scriptsize]
    at ([yshift=-0.85cm]current page.north)
    {Centro de Ciências e Tecnologia};
  \node[white, anchor=north, font=\scriptsize]
    at ([yshift=-1.25cm]current page.north)
    {Programa de Pós-Graduação em Ciência da Computação};
  \node[white, anchor=north, font=\scriptsize]
    at ([yshift=-1.65cm]current page.north)
    {Doutorado em Ciência da Computação $\cdot$ Redes de Computadores};

  %--- título ---
  \node[text width=10.5cm, align=center, anchor=center,
        font=\large\bfseries]
    at ([yshift=0.60cm]current page.center)
    {Modelo de Decisão Baseado em Lógica \textit{Fuzzy}-TOPSIS e\\[4pt]
     Aprendizado por Reforço Profundo\\[4pt]
     para o \textit{Offloading} de Tarefas\\[4pt]
     em Redes Veiculares 5G};

  %--- linha divisória ---
  \draw[blue!80!black, line width=1.2pt]
    ([xshift=-4.5cm, yshift=-1.25cm]current page.center) --
    ([xshift=4.5cm,  yshift=-1.25cm]current page.center);

  %--- autor ---
  \node[anchor=center, font=\normalsize\bfseries]
    at ([yshift=-1.80cm]current page.center)
    {Antônio Sérgio de Sousa Vieira};

  %--- orientador ---
  \node[anchor=center, font=\small]
    at ([yshift=-2.40cm]current page.center)
    {Orientador: Prof. Dr. Joaquim Celestino Júnior (UECE/CCT)};

  %--- co-orientador ---
  \node[anchor=center, font=\small]
    at ([yshift=-2.95cm]current page.center)
  ...
  \node[white, anchor=south, font=\small\bfseries]
    at ([yshift=0.60cm]current page.south)
    {Fortaleza -- Ceará};
  \node[white, anchor=south, font=\small]
...
\begin{frame}[plain]
\begin{center}
\includegraphics[page=1, width=0.82\textwidth, height=0.92\textheight, keepaspectratio]{/home/sergiosvieira/Documentos/2025.2/qualifica/leitura-obrigatoria/FOFF_an_energy-efficient_task_offloading_in_VEC-en.pdf}
\end{center}
\end{frame}
}
...
\begin{frame}[plain]
\begin{center}
\includegraphics[page=1, width=0.82\textwidth, height=0.92\textheight, keepaspectratio]{/home/sergiosvieira/Documentos/2025.2/qualifica/uecetex2/related-papers/section-related-works/literature-review/Deep learning approaches for computation offloading in edge.pdf}
\end{center}
\end{frame}
}
...
\color{black}
\vspace{0.25cm}
\hspace{0.3cm}{\large\bfseries\ttfamily ORIGINALIDADE E POSICIONAMENTO NO ESTADO DA ARTE}
\vspace{0.30cm}

\hspace{0.3cm}\begin{minipage}{0.92\textwidth}
{\scriptsize\ttfamily\setlength{\baselineskip}{1.8\baselineskip}
$\bullet$ \textbf{Contribuição Central:} Arquitetura híbrida SAC\,+\,TOPSIS para
descarregamento de tarefas em redes VEC.\\[4pt]
$\bullet$ \textbf{TOPSIS:} Comprime o espaço de estados variável ($N$ candidatos)
em um vetor compacto de dimensão fixa (6D) --- invariante à escala.\\[4pt]
$\bullet$ \textbf{SAC:} Política estocástica com entropia máxima para lidar com
a não estacionariedade do ambiente veicular.\\[4pt]
$\bullet$ \textbf{Publicado:} \textit{FOFF: an energy-efficient task offloading in
VEC-enabled vehicular networks using fuzzy TOPSIS} [Vieira et~al., 2025].
}
\end{minipage}
\end{frame}
}
...
\color{black}
\vspace{0.25cm}
\hspace{0.3cm}{\large\bfseries\ttfamily LACUNAS NO ESTADO DA ARTE E RESPOSTA DA PROPOSTA}
\vspace{0.30cm}

\hspace{0.3cm}\begin{minipage}{0.92\textwidth}
{\scriptsize\ttfamily\setlength{\baselineskip}{1.8\baselineskip}
\textbf{LACUNA 1 [Miriyala e Chirra, 2026]:}\\
$\quad\bullet$ Literatura usa \textit{flat vectors} de dimensão fixa.\\
$\quad\bullet$ Ignora densidade variável de veículos e mudanças de topologia.\\
$\quad\Rightarrow$ \textbf{RESPOSTA:} TOPSIS como compressor de estado ---
saída \textbf{6D fixo} independente de $N$.\\[8pt]
\textbf{LACUNA 2 [Miriyala e Chirra, 2026]:}\\
$\quad\bullet$ Políticas treinadas de forma estática divergem
em ambientes de alta mobilidade.\\
$\quad\Rightarrow$ \textbf{RESPOSTA:} SAC+TOPSIS com regularização por entropia
previne colapso da política e mantém exploração contínua guiada por múltiplos critérios.
}
\end{minipage}
\end{frame}
}
